...
class Settings extends PureComponent {
    ...
    createActions = (name, canCreate = true, canDelete = true) => {
        let actions = {
            fetchRecords: (options) => {
                ...
                if (!port) {
                    ...
                    api[name].fetch({ port, paging: true, page, pageLength, sorting: true, sortColumn, sortOrder })
                        .then((res) => {
            ...
            updateRecord: (id, record) => {
                ...
                api[name].update(port, id, record)
                    ...
                    .catch((res) => {
                        ...
                        const msg = res.message || fallbackMsg;
        ...
        if (canCreate) {
            actions.createRecord = (record) => {
                const { port } = this.props;
                const actions = this.actions.tools;
    ...
    actions = {
        ...
        wcs: this.createActions('wcs', false, false), /*{
            fetchRecords: (options) => {
                ...
                if (!port) {
                    this.setState({
                        wcs: {
                            ...this.state.wcs,
                            alertMessage: 'Must be connected to edit work coordinate systems.'
                        }
                    });
                } else {
                    const state = this.state.wcs;
                    const {
                        page = state.pagination.page,
                        pageLength = state.pagination.pageLength,
                        sortColumn = state.sorting.sortColumn,
                    ...
                    api.wcs.fetch({ port, paging: true, page, pageLength, sorting: true, sortColumn, sortOrder })
                        .then((res) => {
                            const { pagination, sorting, records } = res.body;
            ...
            updateRecord: (id, options, forceReload = false) => {
                const { port } = this.props;
                const actions = this.actions.wcs;

                api.wcs.update(id, { port, ...options })
                    .then((res) => {
                        actions.closeModal();
                        ...
                        const records = [...this.state.wcs.records];
                        const index = _findIndex(records, { id: id });
            ...
            updateModalParams: (params = {}) => {
                this.setState({
                    wcs: {
                        ...this.state.wcs,
                        modal: {
                            ...this.state.wcs.modal,
                            params: {
                                ...this.state.wcs.modal.params,
                                ...params
                            }
                        }
                    }
        ...
        tools: {
            fetchRecords: (options) => {
                ...
                if (!port) {
                    this.setState({
                        tools: {
                            ...this.state.tools,
                            alertMessage: 'Must be connected to edit tools.'
                        }
                    });
                } else {
                    const state = this.state.tools;
                    const {
                        page = state.pagination.page,
                        pageLength = state.pagination.pageLength,
                        sortColumn = state.sorting.sortColumn,
                    ...
                    api.tools.fetch({ port, paging: true, page, pageLength, sorting: true, sortColumn, sortOrder })
                        .then((res) => {
                            const { pagination, sorting, records } = res.body;
            ...
            updateRecord: (id, record) => {
                ...
                api.tools.update(port, id, record)
                    .then((res) => {
                        actions.closeModal();
                        actions.fetchRecords();
                    })
            ...
            updateModalParams: (params = {}) => {
                this.setState({
                    tools: {
                        ...this.state.tools,
                        modal: {
                            ...this.state.tools.modal,
                            params: {
                                ...this.state.tools.modal.params,
                                ...params
                            }
                        }
                    }
        ...
        pockets: {
            fetchRecords: (options) => {
                ...
                if (!port) {
                    this.setState({
                        pockets: {
                            ...this.state.tools,
                            alertMessage: 'Must be connected to edit pockets.'
                        }
                    });
                } else {
                    const state = this.state.pockets;
                    const {
                        page = state.pagination.page,
                        pageLength = state.pagination.pageLength,
                        sortColumn = state.sorting.sortColumn,
                    ...
                    api.pockets.fetch({ port, paging: true, page, pageLength, sorting: true, sortColumn, sortOrder })
                        .then((res) => {
                            const { pagination, sorting, records } = res.body;
            ...
            createRecord: (options) => {
                const { port } = this.props;
                const actions = this.actions.pockets;

                api.pockets.create({ port: port, ...options })
                    .then((res) => {
                        actions.closeModal();
                        actions.fetchRecords();
                    })
            ...
            updateRecord: (id, options, forceReload = false) => {
                ...
                api.pockets.update(id, { port: port, ...options })
                    .then((res) => {
                        actions.closeModal();
                        ...
                        const records = [...this.state.pockets.records];
                        const index = _findIndex(records, { id: id });
            ...
            updateModalParams: (params = {}) => {
                this.setState({
                    pockets: {
                        ...this.state.pockets,
                        modal: {
                            ...this.state.pockets.modal,
                            params: {
                                ...this.state.pockets.modal.params,
                                ...params
                            }
                        }
                    }
