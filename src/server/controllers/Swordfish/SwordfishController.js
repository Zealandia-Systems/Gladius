...
class SwordfishController {
    ...
    constructor(engine, options) {
        ...
        // Swordfish
        this.runner = new SwordfishRunner();

        this.runner.on('raw', noop);

        this.runner.on('start', (res) => {
            this.emit('serialport:read', res.raw);
            // Swordfish sends 'start' as the first message after
            // power-on, but not when the serial port is closed and
            // then re-opened.  Swordfish has no software-initiated
            ...
            // Swordfish readiness.  On initial power-up, Swordfish might
        ...
        this.queryTimer = setInterval(() => {
            ...
            const zeroOffset = _.isEqual(
                this.runner.getMachinePosition(this.state),
                this.runner.getMachinePosition(this.runner.state)
            );

            // Swordfish settings
            if (this.settings !== this.runner.settings) {
                this.settings = this.runner.settings;
                this.emit('controller:settings', SWORDFISH, this.settings);
            }
    ...
    populateContext(context) {
        const activeState = this.runner.getActiveState();
    ...
    addConnection(socket) {
        ...
        if (!_.isEmpty(this.settings)) {
            // controller settings
            socket.emit('controller:settings', SWORDFISH, this.settings);
        }
        if (!_.isEmpty(this.state)) {
            // controller state
            socket.emit('controller:state', SWORDFISH, this.state);
        }
        if (this.feeder) {
            // feeder status
...
export default SwordfishController;
